/**
 * 댓글 등록폼
 *  - 이벤트버블링을 이용해서 최상위 document객체에 submit핸들러 연결.
 *  - 폼이 재출되기전 submit이벤트가 발생하고, 상위로 전파(bubbling)되서 이 핸들러를 호출
 */
document.addEventListener('submit', (e) => {
    // 정적으로 생성된 폼, 동적으로 생성된 폼 모두 적용
    // matches : HTML에 동일한 요소이지만 클래스명, 아이디명 선택자가 다를 때
    // 요소에 해당 선택자가 있는지 확인하고 true / false 를 반환
...
document.querySelectorAll(".btn-reply").forEach((button) => {
   button.addEventListener('click', (e) => {
       ...
       // afterend 다음 형제 요소 추가
       tr.insertAdjacentHTML('afterend', html); // 다음형제요소에 html추가해달라하기
   },{
       once: true
   });
});